    ...
    : d( qExifValuePrivateSharedNull() )
{
    ...
    {
    case AsciiEncoding:
        d = new QExifUndefinedValuePrivate( QByteArray::fromRawData( "ASCII\0\0\0", 8 ) + value.toLocal8Bit() );
        break;
    case JisEncoding:
    {
...
QByteArray QExifValue::toByteArray() const
{
    ...
    {
    case Ascii:
        return static_cast< const QExifAsciiValuePrivate * >( d.constData() )->value.toLocal8Bit();
    case Undefined:
...
QExifValue QExifImageHeader::readIfdValue(QDataStream &stream, int startPos, const ExifIfdHeader &header) const
{
    ...
    case QExifValue::Ascii:
        if (header.count > 4) {
            ...
            return QExifValue(QString::fromLocal8Bit(ascii.constData(), ascii.size() - 1));
        } else {
            return QExifValue(QString::fromLocal8Bit(header.offsetAscii, header.count - 1));
        }
    case QExifValue::Short:
    {
